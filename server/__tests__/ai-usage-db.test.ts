...
describe("ai_usage concorrenza reale (DB)", { skip: hasDb ? false : "DATABASE_URL non impostata" }, () => {
  ...
  before(async () => {
    ...
    familyId = f!.id;
    // Entitlement PREMIUM attivo: così la quota weekly-meal-plan è 3/settimana
  ...
  test("16 richieste concorrenti su weekly-meal-plan (max 3/settimana) -> esattamente 3 'ok'", async () => {
    const N = 16;
