...
// ===== ASSISTENTE HOME (frase libera → azioni multiple nelle varie sezioni) =====
...
router.post('/:familyId/assistant-parse', authenticate, requireAiEnabled, requireFamilyMember(), async (req: Request, res: Response) => {
  ...
  try {
    const text = typeof req.body?.text === 'string' ? req.body.text.trim() : '';
    if (!text) {
      return res.status(400).json({ error: { code: 'VALIDATION_ERROR', message: 'Scrivi o detta cosa vuoi aggiungere' } });
    }
    if (text.length > 1000) {
      return res.status(400).json({ error: { code: 'VALIDATION_ERROR', message: 'Testo troppo lungo (max 1000 caratteri)' } });
    }
    ...
    // Nomi dei membri per riconoscere gli assegnatari ("per Marco", "tocca a Anna").
    const members = await db.select().from(familyMembers).where(eq(familyMembers.familyId, familyId));
    const memberNames = members.map(m => m.nickname).filter((n): n is string => !!n);
    ...
    const toMemberId = (assigneeName: string | null): string | null => {
      if (!assigneeName) return null;
      const target = assigneeName.trim().toLowerCase();
      const match = members.find(m => (m.nickname || '').trim().toLowerCase() === target);
      return match ? match.id : null;
    };
    const actions = run.value;
    res.json({
      ...actions,
      events: actions.events.map(e => ({ ...e, assigneeMemberId: toMemberId(e.assigneeName) })),
      chores: actions.chores.map(c => ({ ...c, assigneeMemberId: toMemberId(c.assigneeName) })),
    });
  } catch (error) {
    logger.error('Assistant parse error', { error: String(error) });
    sendAiError(res, error, 'Non sono riuscito a capire la richiesta. Riprova con altre parole.');
  }
});
