...
export default function ReportContentScreen() {
  ...
  const { targetType, targetId, familyId } = useLocalSearchParams<{
    targetType: string;
    targetId: string;
    familyId: string;
  }>();
  ...
  const handleSubmit = async () => {
    if (!selectedCategory || !targetType || !targetId || !familyId) return;
    ...
    >
      <View style={styles.header}>
        ...
        <Text style={[styles.title, { color: colors.text }]}>
          Segnala {TARGET_LABELS[targetType || ""] || "Contenuto"}
        </Text>
      </View>
