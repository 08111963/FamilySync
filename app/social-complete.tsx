...
export default function SocialCompleteScreen() {
  ...
  const handleSubmit = async () => {
    setError('');
    if (!signupToken) {
      setError('Sessione di registrazione non valida. Riprova ad accedere.');
      return;
    }
    if (!name.trim()) {
    ...
    if (ageBand === 'under14') {
      setError('Per creare un account da solo/a devi avere almeno 14 anni. Chiedi a un genitore o tutore di creare il tuo profilo nella famiglia.');
      return;
    }
    if (!acceptedTerms) {
    ...
    try {
      ...
      if (msg.includes('scaduta') || msg.includes('completata')) {
        setError('La registrazione è scaduta. Torna indietro e riprova ad accedere.');
      } else {
        setError(msg || 'Registrazione non riuscita. Riprova.');
      }
    } finally {
      ...
      >
        ...
        >
          ...
          <Text style={[styles.subtitle, { color: colors.textSecondary }]}>
            Prima di creare il tuo account abbiamo bisogno di qualche informazione e del tuo consenso esplicito.
          </Text>
          ...
          <Text style={[styles.label, { color: colors.textSecondary }]}>Il tuo nome</Text>
          <TextInput
