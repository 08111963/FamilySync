...
- Implementata sincronizzazione calendario con calendari esterni (due modalità):
  ...
  - Sincronizzazione FACILE dal telefono (l'utente trovava il flusso da PC macchinoso): (1) pulsante "Sincronizza ora" (solo native) copia tutti gli eventi futuri in un calendario dedicato "FamilySync" sul dispositivo — `syncEventsToDeviceCalendar` in lib/device-calendar.ts crea/riusa il calendario, lo SVUOTA (finestra -366/+731gg) e riscrive → idempotente, zero duplicati; filtro "futuri" con data LOCALE (no UTC), pulsante disabilitato finché `isLoading` del FamilyContext; (2) solo iOS: pulsante "Aggiungi al Calendario iPhone" apre il feed come `webcal://` via Linking (sottoscrizione automatica con 1 tocco)
