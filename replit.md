...
- Schermata Bollette con distinzione chiara degli stati (app/(tabs)/bills.tsx):
  ...
  - Due form SEPARATI per nuova bolletta (nessun selettore interno): "/add-bill" = "Nuova Bolletta" (campo Scadenza), "/add-bill?paid=1" = "Registra Bolletta Pagata" (campo "Data di pagamento" default oggi, banner verde, pulsante "Registra pagamento", invia paid:true + paidAt + dueDate=paidDate); il "+" in bills.tsx: filtro Pagate → form pagata, altri filtri → form da pagare, filtro Tutte → Alert con scelta "Da pagare"/"Già pagata"
