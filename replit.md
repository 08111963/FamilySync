...
- Microfoni in Eventi/Faccende/Chat + sincronizzazione faccende→calendario:
  - `VoiceInput` (hold-to-talk esistente) accanto al campo Titolo in `app/add-event.tsx` e `app/add-chore.tsx` e accanto al campo messaggio in `app/(tabs)/chat.tsx` (il testo dettato viene aggiunto al campo, non inviato automaticamente)
