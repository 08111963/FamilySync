...
- Le occorrenze materializzate condividono un `series_id` (uuid, nullable); DELETE con `?scope=series` elimina per seriesId, con fallback legacy (title+rule+time+createdBy, solo righe seriesId NULL) per le serie pre-migrazione. **Why:** cancellare per titolo+regola poteva colpire serie diverse omonime; l'utente cancellava le occorrenze una a una e la Home mostrava ancora le "Cene" future.
...
- Le faccende completate vengono ELIMINATE automaticamente dopo 5 giorni (`COMPLETED_RETENTION_DAYS`, cleanup best-effort nella GET lista). Decisione utente: le "fatte" non devono accumularsi. `completeChore` client fa update ottimistico della cache (la spunta cambia subito; il refetch aggiunge la prossima occorrenza omonima).
...
## Validazione date condivisa
`isRealIsoDate()` in shared/chore-recurrence.ts è il validator unico per date `YYYY-MM-DD` (regex ancorata + componenti calendariali reali). **Why:** date malformate persistite nel DB hanno mandato in 500 il feed ICS pubblico. **How to apply:** ogni nuovo endpoint/parsing che accetta date dall'esterno deve usare questo validator (Zod `.refine(isRealIsoDate)`), non regex ad hoc; i consumer (feed ICS) restano difensivi e skippano righe invalide.
