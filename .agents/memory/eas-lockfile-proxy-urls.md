...
**Override npm pericolosi (build Expo):** un `overrides` globale su pacchetti usati da Metro può rompere `expo export` in publish: metro richiede image-size **1.x** (override → 1.2.1, mai 2.x) e `babel-preset-expo` deve stare esplicito nelle devDependencies (se resta solo annidato sotto expo, Babel non lo trova). Dopo ogni "npm audit fix" di massa, provare subito `npx expo export --platform web`.
