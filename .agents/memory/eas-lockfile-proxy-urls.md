...
**Override npm pericolosi (build Expo):** un `overrides` globale su pacchetti usati da Metro può rompere `expo export` in publish: metro richiede image-size **1.x** (override → 1.2.1, MAI 2.x: già rotto due volte da task di audit sicurezza, la CVE su image-size è trascurabile perché gira solo in build sui nostri asset) e `babel-preset-expo` deve stare esplicito nelle devDependencies (se resta solo annidato sotto expo, Babel non lo trova). Dopo ogni "npm audit fix" di massa, provare subito `npx expo export --platform web`.

**Package firewall blocca versioni vecchie nel lockfile:** il proxy npm di Replit può vietare (403) versioni pinnate nel lockfile (es. basic-ftp 5.x, tar 7.5.6, shell-quote 1.8.3). Se il mandato impone il lockfile esatto: scaricare i tarball da https://registry.npmjs.org (raggiungibile direttamente), `npm cache add *.tgz`, poi `npm ci --prefer-offline`. Attenzione: se node_modules/cache vengono azzerati, l'install fallirà di nuovo.
